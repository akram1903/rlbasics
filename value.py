        ...
        for i in range(self.size):
            for j in range(self.size):
                if np.random.rand() < self.barrier_prob:
                    ...
                    self.maze[self.size-1, self.size-1] = 0  # 1 represents a barrier


    def is_valid_move(self, x, y):
        return 0 <= x < self.size and 0 <= y < self.size and self.maze[x, y] == 0

    def value_iteration(self, discount_factor=0.9, theta=0.4, max_iterations=1000):
    ...
    def calculate_max_value(self, x, y, value_function, discount_factor):
        ...
        for action in range(4):
            ...
            if self.is_valid_move(next_x, next_y):
                max_value = max(max_value, 1 + discount_factor * value_function[next_x, next_y])
